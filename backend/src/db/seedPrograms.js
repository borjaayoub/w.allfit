...
export async function ensurePrograms() {
  ...
  const existing = await db.query("SELECT COUNT(*) FROM programs");
  const count = parseInt(existing.rows[0].count);
  
  if (count >= 8) {
    console.log(`✓ ${count} programs already exist, skipping seed`);
    return;
  }
  
  if (count > 0) {
    console.log(`⚠ Found ${count} existing programs, will add missing ones...`);
  }
  ...
  for (const program of programs) {
    try {
      ...
      console.error(`✗ Error seeding program ${program.title}:`, err.message);
    }
  }

  if (added > 0 || updated > 0) {
    console.log(`✓ ${added} programs added, ${updated} programs updated`);
  } else {
    console.log(`✓ All programs are up to date`);
  }
}
